...
def generate_fractions(max_number):
    fractions = []
...
def generate_fractions_e_o(max_number, startnumerator, startdenominator):
    fractions = []
    for i in range(startnumerator, startnumerator + max_number + 1):
        # Check if i is even
        if i % 2 == 0:
            for j in range(startdenominator, startdenominator + max_number+1):
                if j % 2 != 0:
                    fractions.append((i, j))
        
    return fractions

def generate_fractions_o_o(max_number, startnum):
    fractions = []
    for i in range(startnum, max_number + 1):
        # Check if i is even
        if i % 2 != 0:
            for j in range(1,max_number+1):
                if j % 2 != 0:
...
def generate_fractions_o_e(max_number, startnum):
    fractions = []
    for i in range(startnum, max_number + 1):
        # Check if i is even
        if i % 2 != 0:
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        maxnum = read_integer("Enter max num numerator:")
    if len(sys.argv) > 2:
        startnumerator = int(sys.argv[2])
    else:
    ...
    if len(sys.argv) > 4:
        if '.' in sys.argv[4]:
            grepfloat = float(sys.argv[4])  
        else:
            grepnum = int(sys.argv[4])
            grepfloat = 0
  
    print(f"grepfloat: {grepfloat}")

    getcontext().prec = 50  # Adjust the precision as needed
    mpmath.mp.dps = 50  # Set the precision to 50 digits

    fractions = generate_fractions_e_o(maxnum, startnumerator, startdenominator)
